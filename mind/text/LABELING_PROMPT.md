# Prompt: label a corpus chunk

Paste everything below the `---` line into a chat AI, then attach or paste one chunk file
(`text/chunks/chunk_000.txt`, `chunk_001.txt`, ...). One chunk per conversation; chunks are
independent, so several can be done in parallel by different workers.

Save the reply verbatim to `text/data/labeled/chunk_000.jsonl` (matching the chunk name), then merge:

```
python text/merge_labels.py text/data/labeled/chunk_000.jsonl
```

`merge_labels.py` tolerates stray code fences and commentary, re-joins to the corpus by `id` to
recover the canonical text, and rejects anything that violates the schema, so a partly malformed
answer is safe to merge: just re-run the chunk to fill the gaps.
