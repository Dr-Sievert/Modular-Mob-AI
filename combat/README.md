...
The fighting half of [Modular Mob AI](../README.md): a neural-network brain for Minecraft mobs. A player-shaped mob, the
agent, is driven every tick by a small network (792 inputs, a GRU memory, 11 controls) that runs inside the game in plain
Java, one batched pass per tick for every mob on the same weights. It's trained offline with PyTorch PPO from what the
game recorded, and new weights swap in without the game restarting.
...
The other half is [`../mind`](../mind/README.md): a pure-Python testbed for the behaviour the fight is not. Nothing here
reaches into it, and nothing there reaches into this.

## Quick start

Windows 10 or 11 and a clone of this repository; nothing else has to be installed first. **Every command is run from this
folder**, `combat/`.
...
Setup installs nothing system-wide: a machine without Java 21 or Python gets them unpacked into `.tools\` here, which git
ignores. The scripts find their root from their own folder, so `combat\scripts\test.ps1` works from the repository root
too — but the docs are written for `combat/`.

## Documentation

Everything is in [docs/](docs/README.md): how it works, how to train, test, play and publish, and what was learned along
the way. [CLAUDE.md](CLAUDE.md) is the short version for an AI assistant working in this half, and
[../CLAUDE.md](../CLAUDE.md) holds the rules about the machine that bind both halves — live runs, worktrees, and never
`gradlew --stop`.
...
.tools/     the Java and Python setup.ps1 downloads when a machine has none (not in git)
```

## Development

Open `mod/` in IntelliJ IDEA with Java 21 as the Gradle JVM and the project SDK, or run `mod\gradlew.bat -p mod <task>`
from this folder. Almost everything lives in `mod/common`, compiled against the vanilla game; `mod/fabric` and
`mod/neoforge` hold registration and the per-level tick hook. Common code never reaches into a loader project.

Work in a git worktree, never in a checkout a training run is using, and pass `--no-daemon` to any Gradle you start
yourself. A worktree has no `trainer\.venv` and no terrain library of its own and borrows the main checkout's through
git; see [../CLAUDE.md](../CLAUDE.md) and [docs/findings.md](docs/findings.md).
