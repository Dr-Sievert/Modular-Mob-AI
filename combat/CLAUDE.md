# The combat mod: notes for Claude

This is `combat/`, one of the repository's two halves; the other is [`mind/`](../mind/README.md), and the machine-wide
rules that bind both are in [../CLAUDE.md](../CLAUDE.md). **Read those first** — they are about live training runs,
worktrees and Gradle daemons, and breaking one of them costs hours of somebody else's work. Everything below is about
this half only, and **every command here is run from `combat/`**, not from the repository root.
...
| [../docs/monorepo.md](../docs/monorepo.md) | How `combat/`, `mind/` and `shared/` are laid out, and why |
...
- **Everything runs from `scripts\*.ps1`**, set up once by `scripts\setup.ps1`, **started from `combat/`**. Nothing is
  hardcoded to a machine; paths are found relative to this folder, which is what `scripts\_common.ps1` resolves as `$Root`.
- **Never commit Mojang assets.** Textures come from the local Gradle cache at runtime, or the game jar.
- **`combat\runs\` is not in git.** Rollouts, demos and checkpoints run to gigabytes. Trained networks that matter are copied
  into `combat\models\` with `scripts\publish.ps1` and committed.
- `.mbw`, `.mbr`, `.pt` and `.nbt` are **binary** in the repository's root `.gitattributes`. The default there is
  `* text eol=lf`, which silently corrupts binary files that aren't listed.
...
## Everyday commands (Windows PowerShell, from `combat/`)
...
From the repository root the same command is `combat\scripts\test.ps1`, and so on: the scripts find `$Root` from their own
folder, so they work either way, but everything documented here and in `docs/` is written for `combat/`.
...
- Minecraft 1.21.1, Java 21, MultiLoader: `mod/common` is shared, `mod/fabric` and `mod/neoforge` are thin. Training
  and tests run on Fabric. The Gradle build is `combat/mod`; `mod/settings.gradle` is its root.
- The build anchors everything on `combat/` — `rootProject.projectDir.parentFile` in
  `mod/buildSrc/src/main/groovy/multiloader-loader.gradle` — so `models/`, `runs/`, `trainer/` and `viewer/` resolve inside
  this half. The one exception is `mainCheckout`, which reads the `.git` at `combat/`'s **parent**, the repository root,
  and joins `combat/` back on: that is what lets a worktree borrow the machine's trainer environment and terrain library.
  Don't collapse the two anchors back into one.
