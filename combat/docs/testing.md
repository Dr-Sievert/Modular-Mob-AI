...
| `decisions over 200 records` | `scores` and `shared observation product` both under 1e-5 | the scorer, and that splitting the first layer at column 77 so one decision's candidates share the observation product gives the same number as scoring each from cold |
...
| `theObservationIsTheFrozenLayout` | that this build's column table was held against `shared/models/decisions/layout.json` **as it loaded** - which is what says the check ran rather than quietly finding no file to run against - and then the 77 columns themselves against the mind that filled them, the four focus slots and the six blocks that read zero included |
