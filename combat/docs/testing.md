...
| `parity.ps1` | `parity ok` once per body for the plain network and again for the attended one, logits agreeing to about 1e-6, and `the plain ones agree to the bit at batches 1 to 64` on both arms; then `mind parity ok` | the game runs exactly the network PyTorch trained, **for every body this build has**, plain and attended — the attention over the enemy slots is different arithmetic and not merely a different size — and the forward pass's explicit vector loops give the same bits as its plain ones; and the two models ported from `mind/` answer their frozen records |

### The mind's arm of the parity check

The last arm of `parity.ps1` — `:fabric:mindParity`, `gametest/tools/MindTool` — is the same discipline for the two models
ported from [`mind/`](../../mind/README.md), and it differs from the arms above in one way: it runs **no Python at all**.
`mind/tools/freeze.py` already wrote 200 records per model with the exact numbers its numpy reference gave and committed
them into `shared/models/*/parity.jsonl`, so this side only reads. A retrain regenerates those files and both sides
re-check; see [`mind/docs/port.md`](../../mind/docs/port.md).

| Line | Pass looks like | Proves |
| --- | --- | --- |
| `interpreter over 200 records` | `buckets exact on every record`, `buffers ... the same from cold and from warm`, then the side vector, logits and floats under 1e-5 | the tokenizer, the FNV-1a hashing and the bag sum are the Python featurizer's, and the pass on top of them is the numpy one. The order is not cosmetic: an integer mismatch in the buckets is a tokenizer or hash bug and nothing below it can line up, and the side vector's scalar columns are integer counts, so a difference there is a counting bug and not rounding. **The `buffers` line is the one thing the answer sheet cannot ask**: its longest line is forty words and a net's working buffers start wider than that, so nothing in the 200 records ever outgrows one. That line reads a line longer than any of them twice on the same net — once while the buffers grow, once after — and it is what catches a buffer grown without carrying what it held, a bug that would otherwise appear the first time a player typed a long sentence |
| `decisions over 200 records` | `scores` and `shared observation product` both under 1e-5 | the scorer, and that splitting the first layer at column 69 so one decision's candidates share the observation product gives the same number as scoring each from cold |

Both lines also carry a timing — microseconds per message and per candidate, the best of ten bursts over the fixture's own
records — because a chat line is answered on the server thread and a decision is taken every tenth tick per agent, and
what those cost is a number this build should have to hand.
...
scripts\test.ps1 -League                  246 fights, twice round every league opponent, squad and jockey on normal and on hard; a table of each
...
scripts\league.ps1 -Test                  the trainer's whole unit suite, where the league's own tests live: Elo, the pairings and their shares, the pairing that is never drawn, the pool, reading and resuming results, and beside them the shard reader, the critic, the auxiliary heads and the teacher pull
...
Each test sets up one situation and checks the numbers a player would get. Nine classes, one per concern, most of them
...
the count below is worth knowing: **78**.
...
| `theTeacherStrikesAPackFromTheEdgeOfItsReach` | the other half of the standoff, and the one an average distance cannot say: **where the blows land**. Three zombies walking in, and the teacher's blows land a mean 2.85 to 3.00 blocks off with none or one coming back, against the 2.4 and the three this asks for. A sword reaches three blocks and anything man sized a block and a half, so a blow landed past two and a half is one thrown from ground the zombie could not have struck from |
| `aClearSecondBodyBuysTheKnockbackBlowInAPack`, `aSecondBodyInReachTakesTheKnockbackBlowAway` | one rule read both ways round: a pair of held vindicators, and the sprint blow is thrown when the second of them is five blocks or more clear and never while one stands inside four. Read off **where the second body actually was when each blow landed** rather than where it was put, because both are held and the agent walks |
...
`AgentMindGameTest` (8) - the mind on the entity, and the one seam a changing brain opens. The numbers asserted are
`mind/dwarfsim`'s own, so a failure here means either that the Java has drifted from the Python or that somebody changed
the table without saying so in [../../mind/docs/design.md](../../mind/docs/design.md). Every agent in it is given the
even temperament first, because the modulators are what make two agents react differently to the same event and a test
that wants an exact number has to stand one of them still:

| Test | Checks |
| --- | --- |
| `aMindSurvivesBeingSavedAndLoaded` | feelings, needs, all eight traits, the temperament, an opinion, an episode and who last hit it, out of a save and back. An agent that forgets who hit it every reload has no memory at all, whatever filled it |
| `feelingsFadeNeedsClimbAndOpinionsCool` | the per-tick rates, and that a quarrel is over long before a loss is: anger is all but gone in a hundred mind ticks and grief is still most of the way there. A relationship that has decayed to nothing is dropped |
| `anInsultRaisesAngerAndLowersTrust` | the table as written - anger +0.26, trust -0.10, respect -0.06, hatred +0.11, the speaker venting a little of its own - and twice the magnitude landing twice as hard |
| `aHitLeavesAGrudgeScaledByTheDamage` | a blow that takes a quarter of the bar is remembered at 1.0 and a scratch at half that; being hit costs trust **and** buys respect at once; and an agent four blocks away files the same episode at three fifths of that, as something seen rather than suffered |
| `aWitnessWhoSeesItReactsAndOneWhoCannotDoesNot` | a witness three blocks off takes 45% of the row and remembers it; one across the room **looking the other way** feels nothing, forms no opinion and remembers nothing. The one that cannot see it is looking away rather than merely far off, which is the commonest way to miss something in a world and the case a witness rule gathered by distance would get quietly wrong |
| `aThreatSaidToYouLandsHarderThanOneOverheard` | the `addressed` head as a gate: a threat said to you frightens you, one reported about somebody else is overheard at a witness's fraction, and a request is not an event at all but a standing pull. Nothing calls this path until stage C's chat hook does; it is written and held now so that stage C is a hook and not a port |
| `theObservationIsTheFrozenLayout` | that this build's column table was held against `shared/models/decisions/layout.json` **as it loaded** - which is what says the check ran rather than quietly finding no file to run against - and then the 69 columns themselves against the mind that filled them, the four focus slots and the five blocks that read zero included |
| `theCombatMemorySurvivesASpellUnderAnotherBrain` | the seam: twenty ticks of fighting, twenty under the scripted fighter, and back to the **same array** holding the **same numbers**, counting on from where it stopped. The scripted fighter is never given a vector at all, and the start of a fresh episode still clears what every brain holds without replacing any of it |
...
## The pack suite (`gametest/tests/AgentPackFightGameTest`, `-Psuite=pack`)

Packs of two, three and four zombies and of two and three vindicators, on the hard rung, on natural ground, and over every
loadout in turn. It is the crowd suite's question asked of the other fight a real world has: a **win rate cannot tell a
fighter holding the edge of its reach from one standing in the middle of a pack**, because both average the same distance to
the nearest body. So it prints what a win rate leaves out — how far off its blows landed, how often two of them were within
two blocks, how many blows it took and whether the body that threw each was the nearest, how much of the time its swing was
ready, and which way its feet were going — in a table per pack and a second table by what the agent was carrying.

```
scripts	est.ps1 -Pack                                    300 fights: five packs and ten loadouts, six of each pairing
scripts	est.ps1 -Pack -Weights modelslast8est.mbw     a network in the teacher's place
scripts	est.ps1 -Pack -Arenas 900                         tighter; the pairings go round in turn, so use a multiple of fifty
```

Two of its choices were measured rather than assumed, and both are in the class comment. **The hard rung**, because on the
normal one with a plain sword the teacher won every one of ninety zombie fights and a column that cannot go up says nothing.
**Every loadout**, because the first sitting with a sword alone read 93% to 100% against zombies while the pack bench read
far less: by loadout the same 900 fights read sword 83.3% and crossbow 43.3%, and the crossbow row landed 0.0 blows a fight
and took 3.2. That row is what the drawn weapon's own pack footwork was written for; see [findings.md](findings.md).

Vindicators are in it for one reason: a vindicator covers 0.24 blocks a tick and a body walking backwards covers 0.216, so
the same five packs sit on either side of the one number that decides whether a kite is possible at all.
