...
public class VillagerVindicatorGameTest {
    ...
    /**
     * The default number of arenas to run. Each one is a separate game test, so the framework gives every arena its own
     * plot with spacing around it, runs them independently, and reports each one on its own.
     *
     * This is only the default. The arenas property in gradle.properties overrides it, and is the number to change: an
     * annotation needs a compile time constant, so the build cannot read this field, and a parallel run that cannot see
     * the real count sizes its workers for the wrong one.
     *
     * The framework lays out eight plots per row and never reuses one, so the field grows as this number does: at ten
     * thousand it is a strip of roughly 107 by 18750 blocks that stays in the world for the rest of the run.
     */
    public static final int ARENA_COUNT = 50000;
    ...
    private static final TestDurationStats TIME_TO_DEATH =
            new TestDurationStats("Villager time to death", GameTestTuning.arenasInShard(GameTestTuning.arenaCount(ARENA_COUNT)));

    @GameTest(template = "arena", timeoutTicks = TIMEOUT_TICKS)
    @RepeatGameTest(ARENA_COUNT)
