...
| `theCrowdedViewOfARealWorldIsTheWorldsOwn` | the mob beside an agent keeps its slot with eleven monsters in view, the furthest of twelve candidates is the one that goes without, every position in that slot is the world's own to a hundredth of a block, and `ENEMY_TARGETS_ME` follows the zombie's own target. What a crowded view does to the network itself is [findings.md](findings.md#perception) |
