...
- **Animation:** it comes from the replay. The head follows yaw and pitch, the body lags towards where the mob walks,
  ...
  over at 0 health.
- **Blocks (`B`):** puts block textures on the tops of grass, sand, stone, dirt, snow, leaves, podzol and red sand.
  The replay only records each column's map colour, so these are plausible blocks, not the real ones.
...
Opened from disk, or as an export, the 3D view draws boxes and says why.
...
| B | Block textures in 3D, when served with a Minecraft jar. |
...
- **Metadata:** `run`, `iteration`, `brain`, `worker`, `fight`, `biome`, `outcome`
  (`win` | `loss` | `timeout`), `ticks` (T) and `tickRate`.
- **Terrain:** the top-down `terrain` (`x`, `z`, `width`, `depth`, and `height` and `color` per column, where index
  = dz · width + dx).
...
without `reward`, `biome`, `iteration` or `projectiles`.
